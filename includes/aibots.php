...
add_filter('robots_txt', function ($output, $public) {
    ...
    $block = "\n# AI bots blocked by Site Integrity & Profiler\n";
...
add_action('init', function () {
    ...
    foreach ($blocked as $token) {
        ...
        if (stripos($ua, $token) !== false) {
            ...
            wp_die(esc_html__('403 Forbidden', 'site-integrity-profiler'), esc_html__('403 Forbidden', 'site-integrity-profiler'), ['response' => 403]);
        }
    }
}, 1);
...
add_action('admin_post_wps_save_aibots', function () {
    ...
    wp_safe_redirect(admin_url('admin.php?page=site-integrity-profiler-aibots&saved=1'));
    exit;
});
