...
add_action('wp_ajax_wps_delete_user', function () {
    ...
    if (!$user_id) {
        wp_send_json_error(['message' => __('Invalid user ID', 'site-integrity-profiler')], 400);
    }

    if ($user_id === get_current_user_id()) {
        wp_send_json_error(['message' => __('You cannot delete your own user.', 'site-integrity-profiler')], 400);
    }

    $user = get_userdata($user_id);
    if (!$user) {
        wp_send_json_error(['message' => __('The user does not exist.', 'site-integrity-profiler')], 404);
    }
    ...
    if (!current_user_can('delete_user', $user_id)) {
        wp_send_json_error(['message' => __('You are not allowed to delete this user.', 'site-integrity-profiler')], 403);
    }
    ...
    if (!$ok) {
        wp_send_json_error(['message' => __('Could not delete the user.', 'site-integrity-profiler')], 500);
    }

    wp_send_json_success([
        /* translators: %s: user login name. */
        'message' => sprintf(__('User deleted: %s', 'site-integrity-profiler'), $user->user_login),
        'deleted' => $user_id,
    ]);
});
