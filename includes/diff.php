...
add_action('wp_ajax_wps_show_diff', function () {
    if (!current_user_can('manage_options')) {
        wp_send_json_error(['message' => __('Insufficient permissions', 'site-integrity-profiler')], 403);
    }
    check_ajax_referer('wps_diff_nonce', 'nonce');

    $rel = isset($_POST['path']) ? sanitize_text_field(wp_unslash($_POST['path'])) : '';
    if (!$rel) {
        wp_send_json_error(['message' => __('Invalid path', 'site-integrity-profiler')], 400);
    }
    ...
    if (!$absFile || !is_file($absFile)) {
        ...
        wp_send_json_error(['message' => sprintf(__('File not found: %s', 'site-integrity-profiler'), $rel)], 404);
    }
    $rel = wps_relative_site_path($absFile);
    if (!$rel || !wps_is_core_path($rel)) {
        wp_send_json_error(['message' => __('Only WordPress core files can be compared.', 'site-integrity-profiler')], 400);
    }

    $current = @file_get_contents($absFile);
    if ($current === false) {
        /* translators: %s: file path. */
        wp_send_json_error(['message' => sprintf(__('Could not read the current file: %s', 'site-integrity-profiler'), $rel)], 500);
    }
    ...
    if (!is_array($fetch) || empty($fetch['body'])) {
        wp_send_json_error(['message' => __('Could not fetch the original file.', 'site-integrity-profiler'), 'details' => $GLOBALS['wps_fetch_last_error'] ?? 'n/a'], 500);
    }

    $original = $fetch['body'];

    if (hash('sha256', $current) === hash('sha256', $original)) {
        wp_send_json_success(['path' => $rel, 'diff' => __('No differences found.', 'site-integrity-profiler')]);
    }
...
add_action('wp_ajax_wps_restore_file', function () {
    ...
    if (!$absFile) {
        /* translators: %s: file path. */
        wp_send_json_error(['message' => sprintf(__('File outside the site: %s', 'site-integrity-profiler'), $rel)], 403);
    }
    $rel = wps_relative_site_path($absFile);
    if (!$rel || !wps_is_core_path($rel)) {
        wp_send_json_error(['message' => __('Only WordPress core files can be restored.', 'site-integrity-profiler')], 400);
    }
    ...
    if ($do_backup && is_file($absFile)) {
        ...
        if (!$backup) {
            wp_send_json_error(['message' => __('Could not create the backup. Check permissions; the file was NOT restored.', 'site-integrity-profiler')], 500);
        }
    ...
    if ($ok === false) {
        wp_send_json_error(['message' => __('Could not write the local file. Check permissions.', 'site-integrity-profiler')], 500);
    }

    wp_send_json_success([
        /* translators: %s: file path. */
        'message' => sprintf(__('File restored successfully: %s', 'site-integrity-profiler'), $rel),
...
add_action('wp_ajax_wps_restore_all_files', function () {
    ...
    if (empty($files)) {
        wp_send_json_error(['message' => __('No files were specified to restore.', 'site-integrity-profiler')], 400);
    }
    ...
    if ($do_backup && !$batch) {
        wp_send_json_error(['message' => __('Could not create the backup. Check permissions; nothing was restored.', 'site-integrity-profiler')], 500);
    }

    foreach ($files as $rel) {
        if (!$rel) {
            $errors[$rel] = __('Invalid path', 'site-integrity-profiler');
            continue;
        }
        $absFile = wps_resolve_site_path($rel);
        if (!$absFile) {
            $errors[$rel] = __('File outside the site', 'site-integrity-profiler');
            continue;
        }
        $rel = wps_relative_site_path($absFile);
        if (!$rel || !wps_is_core_path($rel)) {
            $errors[$rel] = __('Only WordPress core files can be restored.', 'site-integrity-profiler');
            continue;
        }
        $fetch = wps_fetch_core_file_from_zip($version, $rel, $locale);
        if (!is_array($fetch) || empty($fetch['body'])) {
            /* translators: %s: value. */
            $errors[$rel] = sprintf(__('Could not fetch the original: %s', 'site-integrity-profiler'), $GLOBALS['wps_fetch_last_error'] ?? '');
            continue;
        }
        ...
        if ($do_backup && is_file($absFile)) {
            if (!wps_backup_file($absFile, $rel, $batch)) {
                $errors[$rel] = __('Could not create the backup; not restored', 'site-integrity-profiler');
                continue;
            }
        }
        ...
        $ok = @file_put_contents($absFile, $fetch['body']);
        if ($ok === false) {
            $errors[$rel] = __('Could not write', 'site-integrity-profiler');
            continue;
        }
        $restored[] = $rel;
    }
...
add_action('wp_ajax_wps_delete_extra', function () {
    ...
    if (!in_array($rel, wps_get_extra_files(), true)) {
        wp_send_json_error(['message' => __('The file is not in the extras list of the last analysis. Run the analysis again.', 'site-integrity-profiler')], 400);
    }
    ...
    if (!wps_delete_file($absFile)) {
        wp_send_json_error(['message' => __('Could not delete the file. Check permissions.', 'site-integrity-profiler')], 500);
    }

    wps_remove_extras_from_analysis([$rel]);
    /* translators: %s: file path. */
    wp_send_json_success(['message' => sprintf(__('File deleted: %s', 'site-integrity-profiler'), $rel), 'deleted' => $rel]);
});
...
add_action('wp_ajax_wps_delete_all_extras', function () {
    ...
    if (empty($extras)) {
        wp_send_json_error(['message' => __('There are no extra files to delete.', 'site-integrity-profiler')], 400);
    }
    ...
    foreach ($extras as $rel) {
        ...
        $absFile = wps_resolve_site_path($rel);
        if (!$absFile || !is_file($absFile)) {
            $errors[$rel] = __('Not found', 'site-integrity-profiler');
            continue;
        }
        if (wps_delete_file($absFile)) {
            $deleted[] = $rel;
        } else {
            $errors[$rel] = __('Could not delete (permissions)', 'site-integrity-profiler');
        }
    }
...
function wps_fetch_core_file_from_zip(string $version, string $relative_path, string $locale = ''): ?array {
    ...
    if (!class_exists('ZipArchive')) {
        $wps_fetch_last_error = __('The ZipArchive class is not available in your PHP version.', 'site-integrity-profiler');
        return null;
    }
    ...
    $cache = wps_plugin_dir_in_uploads('site-integrity-profiler-cache');
    if (!$cache) {
        $wps_fetch_last_error = __('Could not create the cache directory. Check permissions.', 'site-integrity-profiler');
        return null;
    }
    $cache_dir = $cache['dir'];
    ...
    foreach ($urls as $zip_url) {
        ...
        if (!isset($handles[$zip_path])) {
            ...
            if ($zip->open($zip_path) !== true) {
                $errors[] = basename($zip_url) . ': ' . __('corrupt ZIP', 'site-integrity-profiler');
        ...
        $errors[] = basename($zip_url) . ': ' . sprintf(__('does not contain %s', 'site-integrity-profiler'), $relative_path);
    }

    $wps_fetch_last_error = __('Could not fetch the original file.', 'site-integrity-profiler') . ' ' . implode(' | ', $errors);
    return null;
}
