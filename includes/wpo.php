 ...
 */
function wps_wpo_plugins_info(): array {
 ...
 */
function wps_wpo_autoload_stats(int $top = 10): array {
    global $wpdb;

    $cached = wp_cache_get('wps_autoload_stats', 'site-integrity-profiler');
    if (is_array($cached)) {
        return $cached;
    }
    ...
    wp_cache_set('wps_autoload_stats', $stats, 'site-integrity-profiler', MINUTE_IN_SECONDS);
 ...
 */
function wps_wpo_transient_stats(): array {
    global $wpdb;
    $now = time();
 ...
 */
function wps_wpo_cache_info(): array {
 ...
 */
function wps_wpo_env_versions(): array {
    global $wpdb;
 ...
 */
function wps_wpo_cron_info(): array {
...
add_action('wp_ajax_wps_clean_cron_hook', function () {
    ...
    if (!$hook) {
        wp_send_json_error(['message' => __('Invalid hook', 'site-integrity-profiler')], 400);
    }
    if (has_action($hook)) {
        wp_send_json_error(['message' => __('That hook has a registered action; it is not orphaned and will not be removed.', 'site-integrity-profiler')], 400);
    }
    ...
    wp_send_json_success([
        ...
        'message' => sprintf(__('Tasks removed from hook: %s', 'site-integrity-profiler'), $hook),
        'hook'    => $hook,
        'removed' => (int) $removed,
    ]);
});
