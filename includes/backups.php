 ...
 *
 * - 'uploads' / 'uploads_old': dentro de uploads, protegido frente a HTTP.
 *   ('uploads_old' es el nombre previo al renombrado del plugin.)
 * - 'legacy' / 'legacy_old': ubicación de versiones anteriores a la 3.6, en la
 *   raíz del sitio y accesible por HTTP. Se listan para poder purgarlas.
 *
...
function wps_backup_bases(): array {
    ...
    if (empty($upload['error']) && !empty($upload['basedir'])) {
        $up = trailingslashit(wp_normalize_path($upload['basedir']));
        $bases['uploads'] = $up . 'site-integrity-profiler-backups/';
        // Nombre usado antes de renombrar el plugin: se sigue listando para que
        // las copias existentes no queden huérfanas y puedan purgarse.
        $bases['uploads_old'] = $up . 'wp-profiler-security-backups/';
    }
    // Ubicación de versiones anteriores a la 3.6 (dentro de la raíz del sitio).
    $bases['legacy']     = trailingslashit(wp_normalize_path(ABSPATH)) . 'backup_site-integrity-profiler/';
    $bases['legacy_old'] = trailingslashit(wp_normalize_path(ABSPATH)) . 'backup_wp-profiler-security/';
    ...
    foreach (wps_backup_bases() as $store => $base) {
        ...
        foreach ((array) scandir($base) as $entry) {
            ...
            $batches[] = [
                ...
                'count'  => count($files),
                'size'   => $total,
                'legacy' => in_array($store, ['legacy', 'legacy_old'], true), // en la raíz del sitio
            ];
        }
    }
...
add_action('wp_ajax_wps_restore_backup', function () {
    ...
    if (!$source || !is_file($source)) {
        wp_send_json_error(['message' => __('The backup file was not found.', 'site-integrity-profiler')], 404);
    }
    ...
    if (!$target) {
        wp_send_json_error(['message' => __('Invalid destination path.', 'site-integrity-profiler')], 400);
    }
    ...
    wp_send_json_success([
        ...
        'message' => sprintf(__('File restored from backup: %s', 'site-integrity-profiler'), $target_rel),
    ]);
});
...
add_action('wp_ajax_wps_delete_backup_file', function () {
    ...
    if (!wps_delete_file($path)) {
        wp_send_json_error(['message' => __('Could not delete the backup. Check permissions.', 'site-integrity-profiler')], 500);
    }
    ...
    wp_send_json_success(['message' => __('Backup deleted.', 'site-integrity-profiler')]);
});
...
add_action('wp_ajax_wps_delete_backup_batch', function () {
    ...
    if (!$dir || !is_dir($dir)) {
        wp_send_json_error(['message' => __('The backup was not found.', 'site-integrity-profiler')], 404);
    }
...
add_action('wp_ajax_wps_delete_all_backups', function () {
    ...
    if ($deleted === 0 && empty($errors)) {
        wp_send_json_error(['message' => __('There are no backups to delete.', 'site-integrity-profiler')], 400);
    }
